...
table = read.csv(r"(C:\Users\bqueu\Documents\R\ECBC Graphs\N-gram_Uni-gram List--final version - Netherlands_War(Attacks)_Bigram.csv)")
head(table)
```
...
p = table_pivot |>
  ...
  annotate("text", x = 1.7, y = 20, label = "Areas shaded in gray are \n periods when England sent \n troops to the Netherlands.", fontface = "italic", size = 2.3) +
  annotate("rect", xmin = .3, xmax = 3, ymin = 0, ymax = 25, alpha = .25) +
  annotate("rect", xmin = 8, xmax = 9, ymin = 0, ymax = 25, alpha = .25) +
  annotate("rect", xmin = 3.5, xmax = 4.5, ymin = 0, ymax = 25, fill = "red", alpha = .25) +
  labs(
    title = "Discourse Surrounding Netherlands' Attackers Changes When At Peace",
...
ggplotly(p) |>
  layout(annotations = 
    list(x = 1.37, y = 0, text = "
         * N-grams used relate to the \n Netherlands' opponents in \n the 80 Years' War.
