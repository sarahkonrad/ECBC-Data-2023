...
table = read.csv(r"(C:\Users\bqueu\Documents\R\ECBC Graphs\whole corpus lexicon sheets - irish 2.csv)")
head(table)
```
...
p = table_pivot |>
  ...
  geom_line() +
  ylim(-.25, .65) +
  annotate("rect", xmin = 2, xmax = 3, ymin = -.25, ymax = .65, alpha = .4) +
  annotate("text", x = 2.5, y = .57, label = "Tyrone's \n Rebellion", fontface = "italic", size = 3) +
  geom_point() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_color_manual(values = c("#d5b4b4", "#d5cfb4", "#cad5b4", "#cab4d5", "#0033cc", "#ff9900")) +
  labs(
    x = "Date",
