...
table = read.csv(r"(C:\Users\bqueu\Documents\R\ECBC Graphs\subcorpus sheets - spanish 1.csv)")
head(table)
```
...
p = table_pivot |>
  ...
  annotate("text", x = 2, y = .85, label = "King James I \n is coronated.", fontface = "italic", size = 3) +
  geom_vline(xintercept = 8, color = "red", size = 1, linetype = "longdash", alpha = .4) +
  annotate("text", x = 7, y = .85, label = "King Charles I \n is coronated.", fontface = "italic", size = 3) +
  geom_point() +
  ylim(-.2, .9) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_color_manual(values = c("#d5b4b4", "#d5cfb4", "#cad5b4", "#b6d5b4", "#ff9900", "#b4d1d5", "#cab4d5", "#ff3884", "#000000")) +
  labs(
    x = "Date",
