...
table = read.csv(r"(C:\Users\bqueu\Documents\R\ECBC Graphs\subcorpus sheets - virginia 1.csv)")
head(table)
```
...
p = table_pivot |>
  ...
  annotate("text", x = 4.25, y = .6, label = "The Virginia Company \n was founded in 1606.", fontface = "italic", size = 3) +
  geom_vline(xintercept = 7, color = "red", size = 1, linetype = "longdash", alpha = .4) +
  annotate("text", x = 5.75, y = .75, label = "The Virginia Company \n had its charter \n revoked in 1624.", fontface = "italic", size = 3) +
  annotate("rect", xmin = 1.25, xmax = 1.75, ymin = -.15, ymax = .8, alpha = .4, fill = "red") +
  geom_point() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_color_manual(values = c("#d5b4b4", "#cad5b4", "#4bd891", "#b6d5b4", "#d5cfb4", "#ff9900", "#000000")) +
  labs(
    x = "Date",
