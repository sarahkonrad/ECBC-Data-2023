""" author: data+ 2022
this is the file text.py from the 2022 ECBC github.
It aims to clean XML files from EP (NOT EEBO!, or else it will output blank files) 
and implements preliminary standardization and lemmatization. 2023 ECBC Data+ group intends to use/will use this code to
...
def convert(folder, file, textfolder, typeText):
    ...
    with open(os.path.join(textfolder, name + '.txt'), 'w+') as file:
        if typeText == 'text':
            toFile = text(soup)
        elif typeText == 'dedication':
            toFile = dedication(soup)
        file.write(toFile)
